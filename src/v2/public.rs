...
mod unit_tests {
  ...
  fn bench_sign(b: &mut test::Bencher, msg: &str, footer: Option<&str>) {
    ...
    b.iter(|| {
      public_paseto(msg, footer, &as_key).expect("Couldn't generate v2 public paseto")
    })
  }
  ...
  fn bench_verify(b: &mut test::Bencher, msg: &str, footer: Option<&str>) {
    ...
    let key_pkcs8 = Ed25519KeyPair::generate_pkcs8(&sys_rand).expect("Failed to generate pkcs8 key!");
    let as_key = Ed25519KeyPair::from_pkcs8(key_pkcs8.as_ref()).expect("Failed to parse keypair");
    let token = public_paseto(msg, footer, &as_key).expect("Failed to generate token");
    ...
    b.iter(|| {
      verify_paseto(&token, footer, public_key).expect("Couldn't verify v2 public paseto")
    })
  }
